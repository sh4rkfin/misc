...
class Solution(object):
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        if endWord not in wordList:
            return 0
        self._cache = {}
        path = self.ladder_len(beginWord, endWord, set())
        return len(path)

    def ladder_len(self, begin, end, to_avoid):
        candidates = []
        for w in self._wordList:
            if w not in to_avoid:
                if Solution.differ_by_one_letter(begin, w):
                    if w == end:
                        return {begin, end}
                    candidates.append(w)
        min_len = 0
        min_path = None
        for c in candidates:
            path = self._cache.get(c)
            if path and len(path & to_avoid) == 0:
                length = len(path)
            else:
                to_avoid.add(c)
                path = self.ladder_len(c, end, to_avoid)
                length = len(path)
                to_avoid.remove(c)
                self._cache[c] = path
            if (length > 0 and length < min_len) or min_len == 0:
                min_len = length
                min_path = path | {begin}
                if min_len == 2:
                    return min_path
    ...
    @staticmethod
    def differ_by_one_letter(target, candidate):
        diff_count = 0
...
s = Solution()
length = s.ladderLength(beginWord="hit", endWord="cog",
                        wordList=["hot", "dot", "dog", "lot", "log", "cog"])
print('length:', length)
length = s.ladderLength(beginWord="leet", endWord="code",
                        wordList=["lest", "leet", "lose", "code", "lode", "robe", "lost"])
print('length:', length)
length = s.ladderLength(beginWord="qa", endWord="sq",
                        wordList=["si","go","se","cm","so","ph","mt","db","mb","sb","kr","ln","tm","le",
...
length = s.ladderLength(beginWord="cet", endWord="ism",
                        wordList=["kid","tag","pup","ail","tun","woo","erg","luz","brr","gay","sip","kay",
