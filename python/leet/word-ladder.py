...
class Solution(object):
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        if endWord not in wordList:
            return 0
        visited = self.ladder_len(beginWord, endWord)
        if endWord not in visited:
            return 0
        return visited[endWord][1]

    def ladder_len(self, begin, end):
        parent = begin
        to_visit = [parent]
        visited = {begin: (begin, 1, None)}
        not_visited = set(self._wordList)
        idx = 0
        while True:
            ...
            for w in not_visited:
                if Solution.differ_by_one_letter(current, w):
                    if w not in visited:
                        distance = current_info[1] + 1
                        to_visit.append(w)
                        visited[w] = (w, distance, current)
                        to_delete.append(w)
                        if w == end:
                            return visited
            for w in to_delete:
